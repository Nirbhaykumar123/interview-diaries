...
export default function InterviewCard({ interview }: InterviewCardProps) {
  ...
  return (
    <Card className="relative group overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 border-slate-200/80 hover:border-indigo-200/60 hover:bg-indigo-50/5">
      {/* Views count metadata positioned absolutely at the top right */}
      <div className="absolute top-6 right-6 flex items-center gap-1.5 text-xs font-medium text-slate-400 select-none bg-slate-50 border border-slate-100 rounded-full px-2.5 py-1 z-10 shrink-0">
      ...
      <CardContent className="p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <Link
              to={`/interviews/${id}`}
              className="inline-flex items-center gap-1.5 text-base font-bold text-slate-900 hover:text-indigo-600 group-hover:text-indigo-600 transition-colors truncate mt-1 pr-20"
            >
              <span>{role}</span>
